...
**Feature added 2026-07-03 (UX reworked 2026-07-04):** Labels tab — text labels on maps (`cns_map_labels` table, DB v1.2.0; REST `/maps/{id}/labels` + `/labels/{id}`; editor tab with canvas placement/list + context-panel form (text, centered/indicator placement, offsets, bg/border/text color pickers, font size); rendered on the frontend map and the Preview tab). Editor interaction: click a label part to pick it up — it follows the cursor, click drops, Esc cancels; in indicator mode the anchor dot and the text box are draggable independently (box drag = offset only, dot drag = anchor only with box staying put, Reposition button = whole label). Form edits (colors/text/placement/coords) live-preview on the canvas via `onLabelLocalUpdate`; Save persists. `/labels/{id}/position` PATCH accepts any of x/y/offset_x/offset_y (400 on empty). Keyboard (2026-07-05, Labels tab only, suppressed while typing in form fields): Enter drops a carried label / picks up the selected one, Esc cancels/deselects, arrow keys nudge 1 px (Shift = 10 px, debounced PATCH), Ctrl/⌘+C & V copy/paste (internal clipboard, cascading +24 px offsets), Ctrl/⌘+D duplicates, Delete/Backspace deletes (with confirm); Duplicate buttons in the context-panel header and list rows. Infoboxes (2026-07-05): labels support the same infobox model as objects/areas — `linked_post_id`/`infobox_source`/`infobox_data` columns (DB v1.3.0), manual (title/description/image) or post-linked, resolved via `cns_map_suite_resolve_infobox()` in map-data.php with `linked_post_id` cache priming; on the frontend, labels with resolved content are clickable (hit-tested before objects, box + dot) and open the shared drawer, content-less labels stay decorative; LabelForm has the Infobox section (MediaPicker/PostSearch) and ContextPanel mirrors infobox edits into local state so canvas drags don't clobber them. Infobox model reworked 2026-07-05: the connected post (PostSearch) is always available and independent of `infobox_source` — whenever `linked_post_id` is set, `cns_map_suite_resolve_infobox()` puts its permalink in `post_url` and the frontend drawer shows a "Read more →" link; the manual/post radio only picks where title/description/image come from. Applies to objects, areas, and labels; AreaForm also gained the previously missing infobox image picker (backend already supported it). An item whose infobox has only a connected post (no content) still counts as clickable.

**Editor consistency fixes (2026-07-05):** (1) area geometry now persists automatically — canvas node drops, node-list edits, and shape-type switches debounce (600 ms) into `PATCH /areas/{id}/nodes` (nodes and/or shape_type, 400 on empty/invalid); the beforeunload guard also warns while a geometry save is pending; (2) ObjectsCanvas reworked to the labels pick-up/follow-cursor model (click picks up, click/Enter drops, Esc cancels, grab/grabbing cursors) — the old blind teleport-on-empty-click is gone; (3) clicking empty canvas on the Objects tab with nothing selected opens the Add Object modal at that position (makes the long-stale toolbar/list copy true); (4) AreasCanvas Enter-commit now uses the shared `isTypingTarget` guard (moved to `src/admin/utils.ts`) instead of a DOM-class query.

**Unified keyboard layer (2026-07-05):** `src/admin/app/useCanvasKeyboard.ts` — shared `useCanvasKeyboard` hook (⌘/Ctrl+C/V copy & paste with per-panel module clipboards and cascading +24 px pastes, ⌘/Ctrl+D duplicate, Delete/Backspace with confirm, arrow-key nudge with Shift = 10 px; handlers return true to claim preventDefault; suppressed while typing) plus `createDebouncedNudge` for point entities. All three panels use it: labels (refactored onto it), objects (nudge persists via position PATCH through new `handleObjectLocalUpdate`), areas (arrows move the whole area by shifting all nodes; persistence rides the debounced geometry PATCH; paste/duplicate offset normalized nodes by 24 px worth). Esc now also deselects areas (parity). Duplicate buttons added to object/area context-panel headers and list rows (`handleObjectDuplicate` / `handleAreaDuplicate` in MapEditorApp). Enter/Escape drag handling stays in the canvases. Tab-through-nodes for areas (2026-07-05): with an area selected, Tab/Shift+Tab cycles a keyboard-focused node (filled blue handle; hook gained an optional `tab` handler); arrows nudge that node (shape constraints via shared `moveAreaNode` in areas.ts, clamped 0–1) instead of the whole area; Delete removes the focused node where the shape allows (`canRemoveAreaNode`: polygon/bezier, >3 nodes) and walks backwards on repeat; Esc clears node focus before deselecting; clicking a node syncs the focus so Tab continues from there.

**Unification sweep (2026-07-05):** implemented the cross-entity consolidation from the objects/areas/labels comparison.
- *PHP*: `cns_map_suite_infobox_rest_args()` / `cns_map_suite_infobox_fields_from_request()` (single source for the five infobox args + linked-post validation, used by all six create/update handlers) and `cns_map_suite_normalize_row()` behind the three row normalizers.
- *Shared geometry*: `src/shared/map-geometry.ts` — area/region path builders, object/area hit tests, label box math + `drawLabelShape` + `findLabelPartAtPoint` — imported by both the admin bundle (areas.ts/objects.ts/labels.ts re-export) and the frontend `blocks/map/view.js` (webpack entry), so editor and frontend are pixel-identical by construction.
- *Frontend*: per-item clickability everywhere — objects/areas/labels without resolved infobox content are inert and clicks pass through them.
- *React*: shared `<InfoboxSection>` + `infoboxFormDefaults()` (forms extend `InfoboxFormFields`); `useMapResource` hook (all four panels' load effect); generic `EntityTable` behind ObjectsList/AreasList/LabelsList (verbs unified to Edit/Duplicate/Delete); `usePickupDrag` hook now backs both ObjectsCanvas and LabelsCanvas (payload = object id / label part).
- *ObjectModal retired*: objects create-then-edit-in-context-panel like areas/labels — Add Object / click-to-place create a "New Object" immediately and select it; the list's Edit selects.
- *Schema*: labels gained `object_time` (DB v1.4.0, REST arg, "Label Time" form field) for time-slider parity with objects/areas.
Deferred with the api.php split (#6): the generic `cns_map_suite_rest_crud()` handler dedup — pure restructuring, one reviewed change.

**Zoom (2026-07-07):** +/− controls (with % readout) top-right of every map canvas. Implementation: zoom scales the canvas's *display* width (zoom × 100%) inside a scrollable viewport — the canvas backing store and pixel coordinate system are untouched, so all hit-testing/drag editing keeps working (every interaction path normalizes by `getBoundingClientRect`). Zoom changes keep the viewport centered on the same map point; range 100–400% in 10% steps. Editor: shared `CanvasZoomWrap` component wraps all five canvases (Objects/Areas/Labels/Hierarchy/Preview); level persists across tab switches. Frontend map block: `setupZoomControls` in view.js (controls on the block wrapper, `.cns-map-canvas-wrap.is-zoomed` scrolls, hierarchy tooltip unaffected — positions from live rect). Story block: same pattern in cns-story-suite view.js (canvas moved into an injected `.cns-story-canvas-scroll`). Note: zoom is CSS upscaling — sharpness beyond native resolution still awaits the deferred DPR item (#12). Fullscreen (2026-07-07): lightbox-style toggle (⛶/✕ above the zoom buttons) on the frontend map block and the editor Preview tab only (`CanvasZoomWrap allowFullscreen`) — fixed dark overlay (z 99990, under the infobox drawer at 99999; hierarchy tooltip bumped to 99995), body scroll locked, Esc exits (frontend Esc lets an open drawer close first), zoom/pan fully available inside; at 100% the map letterboxes to the screen, zoomed state keeps width-driven scaling. Fixed en route: the story block's `block.json` pointed at `file:./style.scss` with nothing importing it, so the block shipped **no stylesheet at all** — index.js now imports style.scss/editor.scss and block.json references the built `style-index.css`/`index.css`.
