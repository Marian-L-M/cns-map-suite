...
( function () {
	...
	function collectDrawState() {
		return {
			width:
				parseInt(
					document.getElementById( 'cns-map-width' )?.value,
					10
				) || 1000,
			aspectRatio:
				parseFloat(
					document.getElementById( 'cns-map-aspect-ratio' )?.value
				) || 1.0,
			bgType:
				document.querySelector(
					'input[name="cns-map-bg-type"]:checked'
				)?.value || 'color',
			bgColor:
				document.getElementById( 'cns-map-bg-color' )?.value ||
				'#1a1a2e',
			bgImageUrl:
				document.getElementById( 'cns-map-bg-image-id' )?.dataset
					.currentUrl || '',
			imgUrl:
				document.getElementById( 'cns-map-image-id' )?.dataset
					.currentUrl || '',
			imageX:
				parseFloat(
					document.getElementById( 'cns-map-image-x' )?.value
				) || 0,
			imageY:
				parseFloat(
					document.getElementById( 'cns-map-image-y' )?.value
				) || 0,
			imageW:
				parseFloat(
					document.getElementById( 'cns-map-image-width' )?.value
				) || 1.0,
		};
	}
	...
	async function drawMapCanvas( canvasEl, state ) {
		...
		if ( mapImg ) {
			...
			const drawH =
				drawW * ( mapImg.naturalHeight / mapImg.naturalWidth );
			ctx.drawImage(
				mapImg,
				width * state.imageX,
				height * state.imageY,
	...
	function drawAreaShape( ctx, area, W, H, isSelected ) {
		...
		nodes.forEach( function ( node, idx ) {
			...
			const nx =
				isRepoNode && nodeRepoCursor ? nodeRepoCursor.x : node.x * W;
			const ny =
				isRepoNode && nodeRepoCursor ? nodeRepoCursor.y : node.y * H;
	...
	function findNodeAtPoint( ctx, x, y, nodes, W, H ) {
		for ( let i = nodes.length - 1; i >= 0; i-- ) {
			ctx.beginPath();
			ctx.rect(
				nodes[ i ].x * W - NODE_HALF,
				nodes[ i ].y * H - NODE_HALF,
				NODE_HALF * 2,
				NODE_HALF * 2
			);
	...
	async function drawFullCanvas( canvas, objects, areas ) {
		...
		for ( const area of areas )
			drawAreaShape( ctx, area, canvas.width, canvas.height, false );
	...
	async function loadAndDrawPreview() {
		...
		const mapId =
			parseInt(
				document.querySelector( '.cns-map-editor' )?.dataset.mapId,
				10
			) || 0;
		...
		tabs.forEach( function ( tab ) {
			tab.addEventListener( 'click', function () {
				...
				if ( target === 'objects' ) {
					...
					const emptyP = document.querySelector(
						'#cns-context-empty p'
					);
					if ( emptyP )
						emptyP.textContent =
							'Select an object on the canvas to display the sidebar.';
				}
				...
				if ( target === 'areas' ) {
					...
					if ( emptyP )
						emptyP.textContent =
							'Select an area on the canvas  to display the sidebar.';
				}
			} );
		} );
	}
	...
	function initMasterMapToggle() {
		...
		function applyToggle() {
			...
			document
				.querySelectorAll( '[data-master-hide]' )
				.forEach( function ( el ) {
					el.style.display = isMaster ? 'none' : '';
				} );
			document
				.querySelectorAll( '[data-master-show]' )
				.forEach( function ( el ) {
					el.style.display = isMaster ? '' : 'none';
				} );
			...
			if ( activeTab && activeTab.style.display === 'none' ) {
				document
					.querySelector( '.cns-tab[data-tab="settings"]' )
					.click();
			}
		}
	...
	function initEditorCanvas() {
		...
		if ( bgImageIdEl && window.cnsMapEditor )
			bgImageIdEl.dataset.currentUrl =
				window.cnsMapEditor.bgImageUrl || '';

		[
			'cns-map-width',
			'cns-map-aspect-ratio',
			'cns-map-image-x',
			'cns-map-image-y',
			'cns-map-image-width',
		].forEach( function ( id ) {
		...
		document
			.querySelectorAll( 'input[name="cns-map-bg-type"]' )
			.forEach( function ( r ) {
		...
		document
			...
			.forEach( function ( radio ) {
				radio.addEventListener( 'change', function () {
					...
					document
						.querySelectorAll( '.cns-bg-section--color' )
						.forEach( function ( el ) {
							el.classList.toggle( 'cns-hidden', isImage );
						} );
					document
						.querySelectorAll( '.cns-bg-section--image' )
						.forEach( function ( el ) {
							el.classList.toggle( 'cns-hidden', ! isImage );
						} );
				} );
	...
	function initImagePicker() {
		...
		_setupMediaPicker(
			...
			function ( att ) {
				hiddenEl.dataset.currentUrl = att ? att.url : '';
				drawEditorCanvas();
			}
		);
	}
	...
	const objectsCtxHandler = {
		save: async function () {
			...
			try {
				const data = await performSaveObject(
					mapId,
					selectedObjectId,
					panelController.collect(),
					'cns-context-save-status',
					'cns-context-save'
				);
				...
				/* error shown by performSaveObject */
			}
		},
		delete: function () {
		...
		reposition: function () {
			const canvas = document.getElementById( 'cns-objects-canvas' );
			const obj = objectsList.find( function ( o ) {
	...
	function renderObjectsList( objects ) {
		...
		if ( ! objects.length ) {
			container.innerHTML =
				'<p class="cns-objects-empty">No objects yet. Click on the canvas to place one.</p>';
			return;
		}
		...
		const rows = objects
			.map( function ( obj ) {
				const iconHtml = obj.icon_url
					? '<img src="' +
					  esc( obj.icon_url ) +
					  '" width="28" height="28" alt="" style="display:block;object-fit:contain" />'
					: '<span class="cns-obj-dot" style="background:' +
					  esc( obj.canvas_styles?.fillStyle || '#2271b1' ) +
					  '"></span>';
				return (
					'<tr data-id="' +
					obj.id +
					...
					'</td>' +
					'<td>' +
					esc( obj.title || '(no title)' ) +
					'</td>' +
					'<td><span class="cns-badge cns-badge--type">' +
					esc( obj.type ) +
					'</span></td>' +
					'<td>' +
					obj.x +
					...
					'<button class="button button-small cns-obj-edit" data-id="' +
					obj.id +
					'">Edit</button> ' +
					'<button class="button button-small cns-obj-delete" data-id="' +
					obj.id +
					'">Delete</button>' +
					'</td>' +
					'</tr>'
	...
	function buildObjectFormHTML() {
		...
		return (
			...
			'  <label><input type="radio" name="obj-icon-src-' +
			u +
			'" data-field="icon-source" value="svg" checked /> From library</label>' +
			'  <label><input type="radio" name="obj-icon-src-' +
			u +
			'" data-field="icon-source" value="image" /> Custom image</label>' +
			'</div>' +
			'<div data-section="icon-svg">' +
			'  <div class="cns-icon-picker-grid" data-field="icon-picker"></div>' +
			'  <p class="description"><a data-field="icon-library-link" href="' +
			iconsUrl +
			'" target="_blank">Manage icon library →</a></p>' +
			'</div>' +
			...
			'<h3>Infobox</h3>' +
			'<div class="cns-radio-toggle">' +
			'  <label><input type="radio" name="obj-infobox-src-' +
			u +
			'" data-field="infobox-source" value="manual" checked /> Manual</label>' +
			'  <label><input type="radio" name="obj-infobox-src-' +
			u +
			'" data-field="infobox-source" value="post" /> From post</label>' +
			'</div>' +
	...
	function createObjectFormController( root ) {
		...
		function qs( field ) {
			return Array.from(
				root.querySelectorAll( '[data-field="' + field + '"]' )
			);
		}

		function getRadio( field ) {
			return qs( field ).find( function ( r ) {
				return r.checked;
			} )?.value;
		}
		function setRadioVal( field, v ) {
			qs( field ).forEach( function ( r ) {
				r.checked = r.value === v;
			} );
		}
		...
		function populate( obj, x, y ) {
			...
			root.querySelector( '[data-section="icon-svg"]' ).classList.toggle(
				'cns-hidden',
				! isSvg
			);
			root.querySelector(
				'[data-section="icon-image"]'
			).classList.toggle( 'cns-hidden', isSvg );
			localSelectedIconId =
			...
			if ( iconPrev )
				iconPrev.innerHTML =
					obj?.icon_url && ! isSvg
						? '<img src="' + esc( obj.icon_url ) + '" alt="" />'
						: '<span>No image selected</span>';
			const rmIcon = root.querySelector(
				'[data-action="remove-icon-image"]'
			);
			if ( rmIcon )
				rmIcon.classList.toggle(
					'cns-hidden',
					! ( obj?.icon_url && ! isSvg )
				);
			...
			if ( q( 'object-time' ) )
				q( 'object-time' ).value = obj?.object_time ?? 0;
			...
			root.querySelector(
				'[data-section="infobox-manual"]'
			).classList.toggle( 'cns-hidden', ibSrc === 'post' );
			root.querySelector(
				'[data-section="infobox-post"]'
			).classList.toggle( 'cns-hidden', ibSrc !== 'post' );

			// Manual infobox
			if ( q( 'infobox-title' ) )
				q( 'infobox-title' ).value = obj?.infobox_data?.title || '';
			if ( q( 'infobox-desc' ) )
				q( 'infobox-desc' ).value =
					obj?.infobox_data?.description || '';
			...
			const rmIb = root.querySelector(
				'[data-action="remove-infobox-image"]'
			);
			...
			if ( sizeEl ) {
				sizeEl.value = size;
				const out = sizeEl.parentElement?.querySelector(
					'output.cns-range-value'
				);
		...
		function renderIconPicker() {
			...
			if ( ! icons.length ) {
				grid.innerHTML =
					'<p class="description">No icons yet. <a href="' +
					esc( window.cnsMapSuite?.iconsUrl || '#' ) +
					'" target="_blank">Add icons →</a></p>';
				return;
			}
			grid.innerHTML = icons
				.map( function ( icon ) {
					const active =
						icon.id === localSelectedIconId
							? ' cns-icon-item--active'
							: '';
					return (
						'<button type="button" class="cns-icon-item' +
						active +
						'" data-id="' +
						icon.id +
						'" title="' +
				...
				.join( '' );
			grid.querySelectorAll( '.cns-icon-item' ).forEach(
				function ( btn ) {
					btn.addEventListener( 'click', function () {
						localSelectedIconId = parseInt( btn.dataset.id, 10 );
						grid.querySelectorAll( '.cns-icon-item' ).forEach(
							function ( b ) {
								b.classList.remove( 'cns-icon-item--active' );
							}
						);
		...
		function initInteractivity() {
			...
			qs( 'infobox-source' ).forEach( function ( radio ) {
				radio.addEventListener( 'change', function () {
					...
					root.querySelector(
						'[data-section="infobox-manual"]'
					).classList.toggle( 'cns-hidden', isPost );
					root.querySelector(
						'[data-section="infobox-post"]'
					).classList.toggle( 'cns-hidden', ! isPost );
				} );
			...
			if ( libLink && window.cnsMapSuite?.iconsUrl )
				libLink.href = window.cnsMapSuite.iconsUrl;
			...
			const sizeOutput = sizeSlider?.parentElement?.querySelector(
				'output.cns-range-value'
			);
	...
	function initFormPostSearch( root ) {
				...
				try {
					...
					resultsEl.innerHTML = data.length
						? data
								.map( function ( item ) {
									return (
										'<button type="button" class="cns-post-result" data-id="' +
										item.id +
										'" data-title="' +
										esc( item.title ) +
										'">' +
										esc( item.title ) +
										' <span class="cns-post-result__type">' +
										esc( item.subtype ) +
										'</span></button>'
									);
								} )
		...
		if ( resultsEl ) {
			resultsEl.addEventListener( 'click', function ( e ) {
				...
				if ( labelEl ) {
					labelEl.textContent = btn.dataset.title;
	...
	async function commitReposition( canvas, id, x, y ) {
		...
		try {
			...
			const data = await res.json();
			if ( res.ok ) {
				const idx = objectsList.findIndex( function ( o ) {
					return o.id === id;
				} );
	...
	function openObjectModal( obj, x, y ) {
		...
		modal.querySelector( '.cns-modal__title' ).textContent = obj
			? 'Edit Object'
			: 'Add Object';

		if ( modalController ) {
			modalController.populate( obj, x, y );
			_ensureIconCache( function () {
				modalController.renderIconPicker();
			} );
		}
	...
	async function performSaveObject(
		mapId,
		editId,
		payload,
		statusId,
		...
		try {
			...
			if ( editId ) {
				objectsList = objectsList.map( function ( o ) {
					return o.id === editId ? data : o;
				} );
			} else {
	...
	async function deleteObject( id ) {
		...
		try {
			...
			objectsList = objectsList.filter( function ( o ) {
				return o.id !== id;
			} );
			renderObjectsList( objectsList );
	...
	function initObjectModal( mapId ) {
		...
		modal
			.querySelector( '.cns-modal__close' )
			?.addEventListener( 'click', closeObjectModal );
		modal
			.querySelector( '.cns-modal__backdrop' )
			?.addEventListener( 'click', closeObjectModal );
		document
			.getElementById( 'cns-object-cancel' )
			?.addEventListener( 'click', closeObjectModal );

		document
			.getElementById( 'cns-object-save' )
			?.addEventListener( 'click', async function () {
				if ( ! modalController ) return;
				try {
					await performSaveObject(
						mapId,
						editingObjectId,
						modalController.collect(),
						'cns-object-save-status',
						'cns-object-save'
					);
					closeObjectModal();
				} catch {
					/* error displayed by performSaveObject */
				}
			} );

		// Event delegation for list edit/delete.
		document
			.getElementById( 'cns-objects-list' )
	...
	function initContextPanel() {
		...
		document
			.getElementById( 'cns-context-save' )
			?.addEventListener( 'click', function () {
				ctxHandler?.save();
			} );
		document
			.getElementById( 'cns-context-delete' )
			?.addEventListener( 'click', function () {
				ctxHandler?.delete();
			} );
		document
			.getElementById( 'cns-context-close' )
			?.addEventListener( 'click', function () {
				ctxHandler?.close();
			} );
		document
			.getElementById( 'cns-context-reposition' )
			?.addEventListener( 'click', function () {
	...
	function showContextPanel( handler, title, showObj, populate ) {
		...
		if ( statEl ) {
			statEl.textContent = '';
			statEl.className = 'cns-save-status';
		}
	...
	function showContextObject( obj ) {
		showContextPanel( objectsCtxHandler, obj.title, true, function () {
			if ( panelController ) {
				...
				_ensureIconCache( function () {
					panelController.renderIconPicker();
				} );
			}
		} );
	}
	...
	function buildAreaFormHTML() {
		...
		return (
			...
			'<h3>Infobox</h3>' +
			'<div class="cns-radio-toggle">' +
			'  <label><input type="radio" name="area-ib-src-' +
			u +
			'" data-field="infobox-source" value="manual" checked /> Manual</label>' +
			'  <label><input type="radio" name="area-ib-src-' +
			u +
	...
	function createAreaFormController( root ) {
		...
		function populate( area ) {
			...
			if ( q( 'object-time' ) )
				q( 'object-time' ).value = area?.object_time ?? 0;
			...
			if ( q( 'infobox-title' ) )
				q( 'infobox-title' ).value = area?.infobox_data?.title || '';
			if ( q( 'infobox-desc' ) )
				q( 'infobox-desc' ).value =
					area?.infobox_data?.description || '';
			...
			const styles = area?.canvas_styles || {};
			if ( q( 'area-fill' ) )
				q( 'area-fill' ).value = styles.fill || '#2271b1';
			if ( q( 'area-stroke' ) )
				q( 'area-stroke' ).value = styles.stroke || '#2271b1';
			if ( q( 'area-stroke-width' ) )
				q( 'area-stroke-width' ).value = styles.strokeWidth || 2;
			const opEl = q( 'area-fill-opacity' );
			if ( opEl ) {
				opEl.value = styles.fillOpacity ?? 0.3;
				const out = opEl.parentElement?.querySelector(
					'output.cns-range-value'
				);
				if ( out )
					out.textContent = parseFloat( opEl.value ).toFixed( 2 );
			}
		}
		...
		function collect() {
			return {
				...
				style_fill_opacity:
					parseFloat( q( 'area-fill-opacity' )?.value ) || 0.3,
				style_stroke: q( 'area-stroke' )?.value || '#2271b1',
				style_stroke_width:
					parseInt( q( 'area-stroke-width' )?.value, 10 ) || 2,
			};
		}
		...
		function initInteractivity() {
			...
			initFormPostSearch( root );
			const opEl = q( 'area-fill-opacity' );
			const opOut = opEl?.parentElement?.querySelector(
				'output.cns-range-value'
			);
			if ( opEl && opOut ) {
	...
	function renderNodeList( area ) {
		...
		const rows = nodes
			.map( function ( node, idx ) {
				...
				return (
					'<tr>' +
					'<td class="cns-node-num">' +
					( idx + 1 ) +
					'</td>' +
					'<td><input type="number" class="small-text cns-node-x" data-idx="' +
					idx +
					'" value="' +
					xPct +
					'" min="0" max="100" step="0.1" /></td>' +
					'<td><input type="number" class="small-text cns-node-y" data-idx="' +
					idx +
					'" value="' +
					yPct +
					'" min="0" max="100" step="0.1" /></td>' +
					'<td><button type="button" class="button button-small cns-node-del" data-idx="' +
					idx +
					'">&times;</button></td>' +
					'</tr>'
				);
		...
		container
			.querySelectorAll( '.cns-node-x, .cns-node-y' )
			.forEach( function ( input ) {
				input.addEventListener( 'input', function () {
					const idx = parseInt( input.dataset.idx, 10 );
					const a = areasList.find( function ( a ) {
					...
					const val =
						Math.max(
							0,
							Math.min( 100, parseFloat( input.value ) || 0 )
						) / 100;
					if ( input.classList.contains( 'cns-node-x' ) ) {
						a.nodes[ idx ].x = val;
					} else {
						a.nodes[ idx ].y = val;
					}
		...
		container
			.querySelector( '.cns-nodes-add-btn' )
			?.addEventListener( 'click', function () {
				const a = areasList.find( function ( a ) {
					return a.id === selectedAreaId;
				} );
	...
	function renderAreasList( areas ) {
		...
		if ( ! areas.length ) {
			container.innerHTML =
				'<p class="cns-objects-empty">No areas yet. Click "Add Area" to create one.</p>';
			return;
		}
		...
		const rows = areas
			.map( function ( area ) {
				return (
					...
					' nodes</td>' +
					'<td class="cns-maps-actions">' +
					'<button class="button button-small cns-area-select" data-id="' +
					area.id +
					'">Select</button> ' +
					'<button class="button button-small cns-area-delete" data-id="' +
					area.id +
					'">Delete</button>' +
					'</td></tr>'
				);
	...
	function deselectArea() {
		...
		nodeRepoCursor = null;
		document
			.getElementById( 'cns-areas-canvas' )
			?.classList.remove( 'cns-canvas--repositioning' );
		drawAreasCanvas();
		hideContextPanel();
	}
	...
	async function performSaveArea( areaId, payload, statusId, saveBtnId ) {
		...
		try {
			...
			areasList = areasList.map( function ( a ) {
				return a.id === areaId ? data : a;
			} );
	...
	const areasCtxHandler = {
		save: async function () {
			...
			const area = areasList.find( function ( a ) {
				return a.id === selectedAreaId;
			} );
			if ( ! area ) return;
			...
			try {
				const data = await performSaveArea(
					selectedAreaId,
					payload,
					'cns-context-save-status',
					'cns-context-save'
				...
				/* error shown by performSaveArea */
			}
		},
		delete: function () {
			if ( ! selectedAreaId || ! confirm( 'Delete this area?' ) ) return;
			const id = selectedAreaId;
			apiFetch( 'DELETE', '/areas/' + id )
				.then( function ( res ) {
					if ( ! res.ok ) {
						alert( 'Delete failed.' );
						return;
					}
					areasList = areasList.filter( function ( a ) {
						return a.id !== id;
					} );
					renderAreasList( areasList );
					deselectArea();
	...
	function initAreasCanvas( mapId ) {
		...
		canvas.addEventListener( 'click', function ( e ) {
			...
			const selArea = selectedAreaId
				? areasList.find( function ( a ) {
						return a.id === selectedAreaId;
				  } )
				: null;
			if ( selArea ) {
				const nodeIdx = findNodeAtPoint(
					ctx,
					x,
					y,
					selArea.nodes || [],
		...
		document.addEventListener( 'keydown', function ( e ) {
			const areasActive = document.querySelector(
				'[data-panel="areas"].cns-tab-panel--active'
			);
		...
		document
			...
			?.addEventListener( 'click', async function () {
				...
				try {
					...
					if ( ! res.ok )
						throw new Error(
							data.message || 'Failed to create area.'
						);
					areasList.push( data );
					renderAreasList( areasList );
					drawAreasCanvas();
	...
	async function initIconLibraryPage() {
		...
		document
			.getElementById( 'cns-add-icon-btn' )
			?.addEventListener( 'click', function () {
				...
				frame.on( 'select', async function () {
					const att = frame
						...
						.toJSON();
					try {
						const res = await apiFetch( 'POST', '/icons', {
							attachment_id: att.id,
						} );
						const data = await res.json();
						if ( ! res.ok )
							throw new Error(
								data.message || 'Failed to add icon.'
							);
	...
	function renderIconLibraryGrid() {
		...
		if ( ! icons.length ) {
			grid.innerHTML =
				'<p class="cns-icon-library-grid__empty">No icons yet. Click “Add Icon” to upload an SVG.</p>';
			return;
		}
		...
		grid.innerHTML = icons
			.map( function ( icon ) {
				return (
					...
					'<div class="cns-icon-library-item__preview"><img src="' +
					esc( icon.url ) +
					'" alt="' +
					esc( icon.title ) +
					'" /></div>' +
					'<span class="cns-icon-library-item__name">' +
					esc( icon.title ) +
					'</span>' +
					'<button type="button" class="cns-icon-library-item__remove" data-id="' +
					icon.id +
		...
		grid.querySelectorAll( '.cns-icon-library-item__remove' ).forEach(
			function ( btn ) {
				btn.addEventListener( 'click', async function () {
					const id = parseInt( btn.dataset.id, 10 );
					if (
						! confirm(
							'Remove this icon from the library? (The attachment itself is kept.)'
						)
					)
						return;
					try {
						...
						iconLibraryCache = ( iconLibraryCache || [] ).filter(
							function ( i ) {
								return i.id !== id;
							}
						);
	...
	function collectPayload( mapId ) {
		return {
			...
			time:
				parseInt(
					document.getElementById( 'cns-map-time' )?.value,
					10
				) || 0,
			image_id:
				parseInt(
					document.getElementById( 'cns-map-image-id' )?.value,
					10
				) || 0,
			image_x:
				parseFloat(
			...
			is_master:
				document.getElementById( 'cns-map-is-master' )?.checked ??
				false,
			featured:
				document.getElementById( 'cns-map-featured' )?.checked ?? false,
			bg_type:
				document.querySelector(
					'input[name="cns-map-bg-type"]:checked'
				)?.value ?? 'color',
			...
			bg_image_id:
				parseInt(
					document.getElementById( 'cns-map-bg-image-id' )?.value,
					10
				) || 0,
		};
	}
